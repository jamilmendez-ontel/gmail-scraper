-- Gmail Scraper — add package_type_full column to v_package_emails
-- Maps the short package_type codes to full human-readable names for the Excel
-- report. Short codes are kept (column 7) so downstream grouping/filtering still
-- works; the full name is appended at the end of the view's column list because
-- CREATE OR REPLACE VIEW only permits additive column changes.
...
SELECT
    ...
    -- NEW: full human-readable package type name (mapped from short code)
    CASE c.package_type
